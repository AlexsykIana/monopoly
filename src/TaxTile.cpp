...
BoardTile::LandedAction TaxTile::onPlayerLanded(Player &player, GameBoard &board, std::vector<Player *> &allPlayers) {
    ...
    if (player.subtractMoney(taxAmount)) {
        std::cout << "$" << taxAmount << " paid by " << player.getName() << ". Remainig money: $" << player.getMoney()
                << std::endl;
    } else {
